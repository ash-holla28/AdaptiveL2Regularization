...
def specify_regularizers(model,
    ...
    def _init_regularizers(model, kernel_regularization_factor,
        ...
        for item in model.layers:
            if isinstance(item, Model):
                _init_regularizers(item, kernel_regularization_factor,
                                   bias_regularization_factor,
                                   gamma_regularization_factor,
                                   beta_regularization_factor,
                                   use_adaptive_l1_l2_regularizer,
                ...
                if bias_regularization_factor != 0 and hasattr(
                    ...
                    if item.use_bias:
                        item.bias_regularizer = AdaptiveL1L2(
                            amplitude_l2=bias_regularization_factor
                        ) if use_adaptive_l1_l2_regularizer else l2(
                ...
                if beta_regularization_factor != 0 and hasattr(
                    ...
                    if item.center:
                        item.beta_regularizer = AdaptiveL1L2(
                            amplitude_l2=beta_regularization_factor
                        ) if use_adaptive_l1_l2_regularizer else l2(
                            l=beta_regularization_factor)

    # Initialize regularizers
    _init_regularizers(model, kernel_regularization_factor,
