...
class AdaptiveL1L2(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.amplitude_l1 is not None:
            self.l1_regularization_factor = self.add_weight(  # pylint: disable=attribute-defined-outside-init
                name="l1_regularization_factor",
                initializer=Constant(0))
        self.l2_regularization_factor = None  # pylint: disable=attribute-defined-outside-init
        if self.amplitude_l2 is not None:
            self.l2_regularization_factor = self.add_weight(  # pylint: disable=attribute-defined-outside-init
    ...
    def call(self, inputs):  # pylint: disable=arguments-differ
        ...
        if self.l1_regularization_factor is not None:
            regularization += self.amplitude_l1 * K.hard_sigmoid(
                self.omega_l1 * self.l1_regularization_factor) * tf.reduce_sum(
                    tf.abs(inputs))
        if self.l2_regularization_factor is not None:
            regularization += self.amplitude_l2 * K.hard_sigmoid(
                self.omega_l2 * self.l2_regularization_factor) * tf.reduce_sum(
                    tf.square(inputs))
        ...
        for item in model.layers:
            ...
            for regularizer_name in [
                ...
                for variable_name in [
                        "l1_regularization_factor", "l2_regularization_factor"
                ]:
                    regularization_factor = getattr(regularizer, variable_name)
                    if regularization_factor is not None:
                        logs["{}_{}_{}".format(
                            item.name, regularizer_name,
                            variable_name)] = preprocess_function(
                                K.get_value(regularization_factor))
